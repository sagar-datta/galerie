...
export function calculateCursorStyle(city: string) {
  ...
  // Ensure Portugal specifically gets small font size
  const fontSize = formattedCity.toLowerCase() === 'portugal' || formattedCity.length > 8 
    ? SMALL_FONT_SIZE 
    : LARGE_FONT_SIZE;
  ...
  const padding = Math.max(12, Math.min(20, textWidth * 0.12)); // Reduced padding
  const calculatedWidth = Math.min(
    Math.max(MIN_WIDTH, textWidth + ARROW_SPACE + padding),
    MAX_WIDTH
  );

  // Calculate text position with tighter spacing
  const textX = calculatedWidth / 2 + 16;
  const textY = fontSize === LARGE_FONT_SIZE ? 22 : 20;

  return {
    cursor: `url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 ${calculatedWidth} 32' width='${calculatedWidth}' height='32'><rect x='6' y='6' width='${calculatedWidth - 8}' height='26' rx='1' fill='%23EBE9D1'/><rect x='3' y='3' width='${calculatedWidth - 8}' height='26' rx='1' fill='%23FF685B'/><g stroke='%23131313' stroke-linecap='square' stroke-width='3' fill='none'><path d='M20 16h18'></path><path d='m24 11l-6 5l6 5'></path></g><text x='${textX}' y='${textY}' fill='%23131313' text-anchor='middle' font-size='${fontSize}' font-family='Helvetica' font-weight='900' stroke='%23131313' stroke-width='0.5'>${formattedCity}</text></svg>") 16 16, auto`,
  };
}
